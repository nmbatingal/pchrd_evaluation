...
function questions_insert(){
	...
	sql('insert into `questions` set       `question`=' . (($data['question'] !== '' && $data['question'] !== NULL) ? "'{$data['question']}'" : 'NULL') . ', `remarks`=' . (($data['remarks'] !== '' && $data['remarks'] !== NULL) ? "'{$data['remarks']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
...
function questions_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$res = sql("select `id` from `questions` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `session_questions` where `question_id`='".addslashes($id[0])."'", $eo);
	...
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		...
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "session_questions", $RetMsg);
		$RetMsg = str_replace("<Delete>", "<input type=\"button\" class=\"button\" value=\"".$Translation['yes']."\" onClick=\"window.location='questions_view.php?SelectedID=".urlencode($selected_id)."&delete_x=1&confirmed=1';\">", $RetMsg);
		$RetMsg = str_replace("<Cancel>", "<input type=\"button\" class=\"button\" value=\"".$Translation['no']."\" onClick=\"window.location='questions_view.php?SelectedID=".urlencode($selected_id)."';\">", $RetMsg);
		return $RetMsg;
	}
...
function questions_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `questions` set       `question`=' . (($data['question'] !== '' && $data['question'] !== NULL) ? "'{$data['question']}'" : 'NULL') . ', `remarks`=' . (($data['remarks'] !== '' && $data['remarks'] !== NULL) ? "'{$data['remarks']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function questions_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	// process form title
	$templateCode = str_replace('<%%DETAIL_VIEW_TITLE%%>', 'Questions lkp details', $templateCode);
