	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'question_types' => array('Question Types', '', 'table.gif', 'Lookups'),
			'questions' => array('Questions', '', 'table.gif', 'Lookups'),
			'sessions' => array('Sessions', '', 'table.gif', 'Lookups'),
			'speakers' => array('Speakers', '', 'table.gif', 'Lookups'),
			'session_questions' => array('Session Questions', '', 'table.gif', 'Lookups'),
			'responses' => array('Responses', '', 'table.gif', 'Data'),
			'respondents' => array('Respondents', '', 'table.gif', 'Data')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'question_types' => "`question_types`.`id` as 'id', `question_types`.`type` as 'type', `question_types`.`remarks` as 'remarks'",
			'questions' => "`questions`.`id` as 'id', `questions`.`question` as 'question', `questions`.`remarks` as 'remarks'",
			'sessions' => "`sessions`.`id` as 'id', `sessions`.`session_name` as 'session_name', `sessions`.`remarks` as 'remarks', `sessions`.`has_token` as 'has_token'",
			'speakers' => "`speakers`.`id` as 'id', `speakers`.`full_name` as 'full_name', IF(    CHAR_LENGTH(`sessions1`.`session_name`), CONCAT_WS('',   `sessions1`.`session_name`), '') as 'session_id'",
			'session_questions' => "`session_questions`.`id` as 'id', IF(    CHAR_LENGTH(`sessions1`.`session_name`), CONCAT_WS('',   `sessions1`.`session_name`), '') as 'session_id', IF(    CHAR_LENGTH(`questions1`.`question`), CONCAT_WS('',   `questions1`.`question`), '') as 'question_id', IF(    CHAR_LENGTH(`question_types1`.`type`), CONCAT_WS('',   `question_types1`.`type`), '') as 'question_type', `session_questions`.`order` as 'order'",
			'responses' => "`responses`.`id` as 'id', IF(    CHAR_LENGTH(`respondents1`.`name`), CONCAT_WS('',   `respondents1`.`name`), '') as 'respondent_id', IF(    CHAR_LENGTH(`questions1`.`question`), CONCAT_WS('',   `questions1`.`question`), '') as 'question_id', `responses`.`response` as 'response'",
			'respondents' => "`respondents`.`id` as 'id', `respondents`.`name` as 'name', `respondents`.`institution` as 'institution'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false) {
		$sql_from = array(   
			...
			'questions' => "`questions` ",
			'sessions' => "`sessions` ",
			'speakers' => "`speakers` LEFT JOIN `sessions` as sessions1 ON `sessions1`.`id`=`speakers`.`session_id` ",
			'session_questions' => "`session_questions` LEFT JOIN `sessions` as sessions1 ON `sessions1`.`id`=`session_questions`.`session_id` LEFT JOIN `questions` as questions1 ON `questions1`.`id`=`session_questions`.`question_id` LEFT JOIN `question_types` as question_types1 ON `question_types1`.`id`=`session_questions`.`question_type` ",
			'responses' => "`responses` LEFT JOIN `respondents` as respondents1 ON `respondents1`.`id`=`responses`.`respondent_id` LEFT JOIN `session_questions` as session_questions1 ON `session_questions1`.`id`=`responses`.`question_id` LEFT JOIN `questions` as questions1 ON `questions1`.`id`=`session_questions1`.`question_id` ",
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> evaluation</a>
			</div>
