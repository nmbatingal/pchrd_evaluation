		...
		$pcConfig = array(
			...
			'speakers' => array(   
				'session_id' => array(   
					...
					'tab-label' => 'Speakers <span class="hidden child-label-speakers child-field-caption">(Session id)</span>',
					'auto-close' => false,
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Speaker', 2 => 'Session id'),
					'display-field-names' => array(1 => 'full_name', 2 => 'session_id'),
					'sortable-fields' => array(0 => '`speakers`.`id`', 1 => 2, 2 => '`sessions1`.`session_name`'),
					'records-per-page' => 10,
					...
					'template' => 'children-speakers',
					'template-printable' => 'children-speakers-printable',
					'query' => "SELECT `speakers`.`id` as 'id', `speakers`.`full_name` as 'full_name', IF(    CHAR_LENGTH(`sessions1`.`session_name`), CONCAT_WS('',   `sessions1`.`session_name`), '') as 'session_id' FROM `speakers` LEFT JOIN `sessions` as sessions1 ON `sessions1`.`id`=`speakers`.`session_id` "
				)
			),
			'session_questions' => array(   
				'session_id' => array(   
					...
					'child-primary-key' => 'id',
					'child-primary-key-index' => 0,
					'tab-label' => 'Questions <span class="hidden child-label-session_questions child-field-caption">(Session id)</span>',
					'auto-close' => false,
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Session id', 2 => 'Question', 3 => 'Question type', 4 => 'Order'),
					'display-field-names' => array(1 => 'session_id', 2 => 'question_id', 3 => 'question_type', 4 => 'order'),
					'sortable-fields' => array(0 => '`session_questions`.`id`', 1 => '`sessions1`.`session_name`', 2 => '`questions1`.`question`', 3 => '`question_types1`.`type`', 4 => '`session_questions`.`order`'),
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-session_questions',
					'template-printable' => 'children-session_questions-printable',
					'query' => "SELECT `session_questions`.`id` as 'id', IF(    CHAR_LENGTH(`sessions1`.`session_name`), CONCAT_WS('',   `sessions1`.`session_name`), '') as 'session_id', IF(    CHAR_LENGTH(`questions1`.`question`), CONCAT_WS('',   `questions1`.`question`), '') as 'question_id', IF(    CHAR_LENGTH(`question_types1`.`type`), CONCAT_WS('',   `question_types1`.`type`), '') as 'question_type', `session_questions`.`order` as 'order' FROM `session_questions` LEFT JOIN `sessions` as sessions1 ON `sessions1`.`id`=`session_questions`.`session_id` LEFT JOIN `questions` as questions1 ON `questions1`.`id`=`session_questions`.`question_id` LEFT JOIN `question_types` as question_types1 ON `question_types1`.`id`=`session_questions`.`question_type` "
				)
			),
			'responses' => array(   
				'question_id' => array(   
					'parent-table' => 'session_questions',
					'parent-primary-key' => 'id',
					'child-primary-key' => 'id',
					'child-primary-key-index' => 0,
					'tab-label' => 'Responses <span class="hidden child-label-responses child-field-caption">(Question id)</span>',
					'auto-close' => false,
					'display-refresh' => true,
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => array(1 => 'Name', 2 => 'Question id', 3 => 'Response'),
					'display-field-names' => array(1 => 'respondent_id', 2 => 'question_id', 3 => 'response'),
					'sortable-fields' => array(0 => '`responses`.`id`', 1 => '`respondents1`.`name`', 2 => 3, 3 => 4),
					'records-per-page' => 10,
					'default-sort-by' => false,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					'display-page-selector' => true,
					'show-page-progress' => true,
					'template' => 'children-responses',
					'template-printable' => 'children-responses-printable',
					'query' => "SELECT `responses`.`id` as 'id', IF(    CHAR_LENGTH(`respondents1`.`name`), CONCAT_WS('',   `respondents1`.`name`), '') as 'respondent_id', IF(    CHAR_LENGTH(`questions1`.`question`), CONCAT_WS('',   `questions1`.`question`), '') as 'question_id', `responses`.`response` as 'response' FROM `responses` LEFT JOIN `respondents` as respondents1 ON `respondents1`.`id`=`responses`.`respondent_id` LEFT JOIN `session_questions` as session_questions1 ON `session_questions1`.`id`=`responses`.`question_id` LEFT JOIN `questions` as questions1 ON `questions1`.`id`=`session_questions1`.`question_id` "
				)
			),
			'respondents' => array(   
			)
